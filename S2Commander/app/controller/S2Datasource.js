Ext.define('SeamlessC2.controller.S2Datasource', {
    ...
    init: function() {        
        ...
        if(OWF.Util.isRunningInOWF()) {           
            ...
            //widgets on current frame
            Ozone.eventing.getAllWidgets(function(widgetList){ 
                log("Frame WidgetList",widgetList);            
                var proxy = Ozone.eventing.importWidget(widgetList[3].id,function (prox) {
                    log("Proxy:",proxy);
                    var widget = proxy.sendMessage('{addr:null}');
                    log("Proxy widget:",widget);
                });
            });
    ...
    onWidgetIntentSelect:function(widget,datasource){
        if(widget.intents && widget.intents.receive[0]){
            ...
            log("Waiting for widget to load",widget);
            var proxy = Ozone.eventing.importWidget(widget.guid,function (prox) {
                    log("Proxy:",proxy);
                    //var widget = proxy.sendMessage('{addr:null}');
                    log("Datasource Sending message on  "+channel,params)  ;               
